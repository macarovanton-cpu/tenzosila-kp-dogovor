...
| MEDIUM | `percent_to_retail` | pricing.py:188 | Нигде не вызывается — мёртвый код? |
...
- **Мёртвый код:** `percent_to_retail()` в pricing.py:188 — определена, нигде не вызывается
...
| 2 | Решить судьбу `percent_to_retail` (удалить или покрыть тестом) | MEDIUM | 5 мин |
...
| Мёртвый код | ⚠️ 1 функция (percent_to_retail) |
