...
## Тесты
- tests/contracts/synthetic/ — ЗАВЕДОМО СЛОМАНЫ, фикстуры дрейфуют.
  НЕ прогонять, НЕ чинить, НЕ анализировать их падения/diff.
  Полный прогон: pytest (synthetic исключён через pytest.ini).
- Если synthetic-фикстуры (case_*) всплыли как modified в git —
  это дрейф бинарников, git restore, не коммитить.
