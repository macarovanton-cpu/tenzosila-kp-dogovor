...
# Вставка-аббревиатура между ОПФ и кавычками (WHITELIST): ГК (группа компаний),
# ТД (торговый дом), ПКФ, НПО, НПП, ТПК, ПФ, КБ. Часть зарегистрированного
# наименования — сохраняем целиком, НЕ вырезаем (усечение = неверное юр. имя).
# Только whitelist, а не «любое слово», иначе «ООО Крутая «Фирма»» нахватает мусор.
_NAME_INSERT = r"ГК|ТД|ПКФ|НПО|НПП|ТПК|ПФ|КБ"

# Наименование организации: ОПФ + опц. вставка-аббревиатура + кавычки
# (жадный захват содержимого кавычек)
_NAME_RE = re.compile(
    r'(ООО|ПАО|ЗАО|АО|ИП)\s*(?:(' + _NAME_INSERT + r')\s+)?["«“]([^"»”]{1,120})["»”]',
    re.IGNORECASE,
)
...
def _extract_name(scan_text: str, result: dict[str, str]) -> None:
    ...
    if m:
        opf = m.group(1).upper()
        insert = m.group(2)  # вставка-аббревиатура (ГК/ТД/…) или None
        name = m.group(3).strip()
        prefix = f"{opf} {insert.strip()}" if insert else opf
        result["ЗАКАЗЧИК_КРАТКОЕ_НАИМЕНОВАНИЕ"] = f'{prefix} "{name}"'
        return
