...
1b. Варианты опор 3х/4х на 20-метровых моделях: 3х выведен из продаж с 2026,
парсер берёт только 4х. Игнор 3х реализован явно (не зависит от порядка строк в PDF).
1c. ОРИОН — одной суммой (оборудование + шеф-монтаж), один ключ на уровень
(orion_lite, orion_standard, ...), флаг individual_calc. НЕ разделять на компоненты:
монтаж ОРИОНа клиент самостоятельно не выполняет (случаев нет), оборудование и
шеф-монтаж всегда идут вместе. Это сознательное ОБЪЕДИНЕНИЕ (в отличие от опор 3х/4х,
где сознательное разделение) — не считать склейку багом. Отдельная спецификация ОРИОН
с разбивкой — будущая фаза, сейчас не реализуется. Цена справочная (individual_calc):
реальный расчёт ОРИОН индивидуален, из прайса берётся как ориентир.
