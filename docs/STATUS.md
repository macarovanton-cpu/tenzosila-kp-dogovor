...
**Текущая фаза:** Фаза 1 — шаги 1.1–1.4 + брендирование + UX + 1.5a + 1.5b + 1.5b-fix + 1.5b-fix2 + 1.5b-fix3 + 1.5b-fix4 + микро-фикс шрифт «Срок действия» (DOCX-генерация) закрыты. Следующий шаг — 1.6 (тестирование на реальных КП) и 1.7 (деплой).
**Готовность Фазы 1:** ~87%
