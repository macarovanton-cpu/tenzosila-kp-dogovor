...
4. Ничего из 1–3 → выбор менеджера. Дефолт зависит от наличия монтажа:
   - **Есть монтаж / шеф-монтаж** (installation_scope ∈ {full, shefmontazh})
     → дефолт `customer_builds`. Монтаж = надёжный сигнал, что основание нужно;
     если его не строит Тензосила — строит Заказчик.
   - **Нет монтажа** (installation_scope == none) → дефолт `none`. Случай
     неоднозначен: чистая поставка / весы на готовый фундамент / только поверка.
     Не зашумляем договор клозами про фундамент. Менеджер выбирает вручную.

   **Обоснование (решение 2026-06-13, отклонение от первоначального «дефолт
   customer_builds ко всем»):** чистая поставка по-хорошему должна жить в
   отдельном договоре поставки (отдельная фаза, ещё не внедрён). Привязка
   дефолта к монтажу — мост, который остаётся корректным и после внедрения
   договора поставки: правило в spec-договоре переделывать не придётся.
...
- **Заказчик строит фундамент** → `customer_builds`
  (дефолт при наличии монтажа/шеф-монтажа)
- **Готовый фундамент Заказчика** → `existing_foundation` (новый scope, ноль
  обязательств по фундаменту + оговорка в договоре)
- **Нет работ по фундаменту** → `none`
  (дефолт при отсутствии монтажа)
...
| нет позиции фунд./рамы + есть монтаж | дефолт `customer_builds` |
| нет позиции фунд./рамы + нет монтажа | дефолт `none`, клозов про фундамент нет |
