...
⚠️ **B-нумерация в §4 — ЛОКАЛЬНАЯ для этого документа** (предложения аудита).
Она НЕ совпадает с ID в `docs/TECH_DEBT.md`. При переносе пункта в долг —
присваивать СВОБОДНЫЙ ID из TECH_DEBT, номер отсюда не переносить.
Занято на 2026-07-13: TECH_DEBT **B11** (композиция `installation_object`),
**B12** (роутинг custom-позиций КП↔Договор).
