...
def test_supply_contract_total_scales_with_qty():
    ...
    # Количество — в ТОВАР_НАИМЕНОВАНИЕ_КОЛВО (§1.1 договора, позиция прил.№1);
    # ТОВАР_НАИМЕНОВАНИЕ (заголовки ТТХ/приложения) — без количества.
    assert "1шт." in c1["ТОВАР_НАИМЕНОВАНИЕ_КОЛВО"]
    assert "2шт." in c2["ТОВАР_НАИМЕНОВАНИЕ_КОЛВО"]
    assert "шт." not in c1["ТОВАР_НАИМЕНОВАНИЕ"]
    assert "шт." not in c2["ТОВАР_НАИМЕНОВАНИЕ"]
