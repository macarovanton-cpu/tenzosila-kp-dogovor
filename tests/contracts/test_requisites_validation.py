...
def _valid_fields() -> dict[str, str]:
    ...
    return {
        ...
        "ЗАКАЗЧИК_РС": "40702810900000012344",
...
def test_rs_valid_checksum_clean():
    """Валидная пара БИК ↔ р/с (контрольный ключ сходится) — нет error по счёту."""
    fields = _valid_fields()
    fields["ЗАКАЗЧИК_РС"] = "40702810000000000007"  # ключ с БИК 044525225 == 0
    errors, _ = validate_requisites(fields)
    assert not any("контрольного ключа" in e for e in errors)


def test_rs_broken_checksum_blocks():
    """Р/с верной длины 20, но с битым контрольным ключом → error."""
    fields = _valid_fields()
    fields["ЗАКАЗЧИК_РС"] = "40702810000000000008"  # ключ ломается
    errors, _ = validate_requisites(fields)
    assert any("контрольного ключа" in e for e in errors)


def test_rs_checksum_not_computed_without_bik():
    """Р/с верной длины при пустом БИК: контрольный ключ не считаем
    (нет опорного БИК), error по ключу не появляется."""
    fields = _valid_fields()
    fields["ЗАКАЗЧИК_БИК"] = ""
    fields["ЗАКАЗЧИК_РС"] = "40702810000000000008"  # ключ был бы бит при БИК
    errors, _ = validate_requisites(fields)
    assert not any("контрольного ключа" in e for e in errors)
...
class TestSyntheticCards:
    def test_clean_card_full_jurlico(self):
        ...
        text = (
            ...
            "р/с 40702810900000012344 в Банк ПАО «Сбербанк», г. Москва "
    ...
    def test_clean_card_unlabeled_accounts(self):
        ...
        text = (
            ...
            "40702810700000098760 Банк: РНКБ Банк (ПАО) БИК 044525607 "
    ...
    def test_gk_insert_name_unblocks_generation(self):
        """Spike: ОПФ + вставка «ГК» + кавычки — наименование распознаётся
        целиком (со «ГК»), полный пайплайн parse→validate НЕ даёт error
        «наименование пусто» (генерация разблокирована)."""
        text = (
            "ООО ГК «ЕвроАгроТранс»\n"
            "ИНН 7707083893 КПП 770701001\n"
            "ОГРН 1027700132195\n"
            "Юридический адрес: 117312, г. Москва, ул. Вавилова, д. 19\n"
            "р/с 40702810900000012344 Банк: ПАО Сбербанк "
            "к/с 30101810400000000225 БИК 044525225\n"
            "Директор Иванов Иван Иванович, действует на основании Устава\n"
        )
        full, errors, _ = _parse_and_validate(text)
        assert full.get("ЗАКАЗЧИК_КРАТКОЕ_НАИМЕНОВАНИЕ") == 'ООО ГК "ЕвроАгроТранс"'
        assert not any("наименование" in e for e in errors)
