...
class TestAddresses:
    ...
    def test_multiline_address_joined(self):
        """Продолжение с хвостовой запятой присоединяется, метка-строка обрывает (P1-5)."""
        text = (
        ...
        assert result.get("ЗАКАЗЧИК_АДРЕС_ЮР") == (
            "630007, Новосибирская область, г. Новосибирск, Красный проспект, д. 1"
        )

    def test_multiline_stops_before_label_line(self):
        """Строка-метка другого поля не поглощается даже после запятой (P1-5)."""
        text = (
            "Юридический адрес: 117997, г. Москва,\n"
            "ИНН 7707083893\n"
        )
        result = parse_requisites(text)
        assert result.get("ЗАКАЗЧИК_АДРЕС_ЮР") == "117997, г. Москва"
        assert result.get("ЗАКАЗЧИК_ИНН") == "7707083893"

    def test_multiline_poct_next_line_not_consumed(self):
        """«Почтовый адрес:» на следующей строке — отдельный кандидат (P1-5)."""
        text = (
            "Юридический адрес: 117997, г. Москва, ул. Вавилова, д. 5,\n"
            "Почтовый адрес: 119991, г. Москва, ул. Ленина, д. 10\n"
        )
        result = parse_requisites(text)
        assert result.get("ЗАКАЗЧИК_АДРЕС_ЮР") == (
            "117997, г. Москва, ул. Вавилова, д. 5"
        )
        assert result.get("ЗАКАЗЧИК_АДРЕС_ПОЧТ") == (
            "119991, г. Москва, ул. Ленина, д. 10"
        )

    def test_multiline_glued_label_cut_by_addr_end(self):
        """Склейка + прилипшая метка в середине строки → режется _ADDR_END (P0-3 поверх P1-5)."""
        text = (
            "Юридический адрес: 117997, г. Москва,\n"
            "ул. Вавилова, д. 5 ИНН 7707083893\n"
        )
        result = parse_requisites(text)
...
# в репо). Ассерты — на зафиксированное аудитом поведение P0, отсутствие
# порчи и закрытые P1-пробелы (P1-1…P1-5: р/сч 408, ФИО «Фамилия И.О.»,
# должность с прилагательным, ИП/полная ОПФ, многострочный адрес).
# ---------------------------------------------------------------------------
...
class TestAuditCards:
    ...
    def test_card4_pao_multiline_address(self):
        """Карточка 4 (полная ОПФ, адрес в 3 строки): имя (P1-4) и полный адрес (P1-5)."""
        text = (
        ...
        assert result.get("ЗАКАЗЧИК_АДРЕС_ЮР") == (  # P1-5
            "630007, Новосибирская область, г. Новосибирск, Красный проспект, д. 1"
        )
