"""Загрузка и валидация JSON-фикстур autoverify-харнесса.

Фикстура = входная карточка сделки: KP-state (как в st.session_state КП),
реквизиты заказчика и параметры договора. Формат зеркалит продакшн:
kp_state → build_kp_snapshot() → kp_row, как при сохранении КП в Supabase.
"""
...
class Fixture:
    ...
    etalon_md: str                    # путь к эталонному .md (справочно)
