...
C4 — сторож двух форм имени подрядчика по контексту. ВАЖНО: эмпирически
текущий код ВСЕГДА использует форму «ТПК «Тензосила»» — форма «Компания
«Тензосила»» (для блока Госреестра/сертификации) нигде не встречается ни в
одном сгенерированном документе. Это противоречит примечанию FIX_SPEC
(«сейчас, судя по эталону, используются правильно») — эталон описывает
ЖЕЛАЕМОЕ поведение, а не то, что уже воспроизводит код. Строка Госреестра
запечена статикой в data/спецификация-шаблоне (ООО «ТПК «Тензосила») —
см. src/contracts/supplier.py docstring. Ловушка ниже красная уже сегодня.
"""
