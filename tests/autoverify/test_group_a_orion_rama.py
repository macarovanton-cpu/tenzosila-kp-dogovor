...
A1: ОРИОН не теряется полностью, но пакуется в одну позицию вместо
2 (монтаж+ОРИОН) / 3 (фундамент+ОРИОН) — «Монтаж ОРИОН» и авто-опоры не
реализованы отдельными позициями.
"""
...
_ORION_POSITION_COUNT = {
    "orion": 3,               # фундамент+ОРИОН: ПАК + монтаж ОРИОН + опоры
